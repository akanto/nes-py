...
class ROM(object):
    ...
    def prg_rom_size(self):
        ...
        return 16 * np.uint32(self.header[4])

    @property
    def chr_rom_size(self):
        """Return the size of the CHR ROM in KB."""
        return 8 * np.uint32(self.header[5])
    ...
    def prg_rom_stop(self):
        ...
        return np.uint32(self.prg_rom_start) + np.uint32(self.prg_rom_size) * np.uint32(2**10)
    ...
    def chr_rom_stop(self):
        ...
        return np.uint32(self.chr_rom_start) + np.uint32(self.chr_rom_size) * np.uint32(2**10)
